...
/** Original procedural foley and a quiet, original sixteen-bar music loop. */
export function createAudio(): AtelierAudio {
  ...
  const lastEvent = new Map<SoundEvent, number>();
  ...
  function play(event: SoundEvent, intensity = 0.6) {
    if (
      !context ||
      context.state !== 'running' ||
      settings.muted ||
      ...
      return;
    const t = context.currentTime;
    const minGap = event === 'impact' || event === 'collision' ? 0.028 : 0.05;
    ...
    switch (event) {
      ...
      case 'impact':
        ...
        break;
      case 'collision':
        noiseHit(t, 0.17 * strength, 3400 * variation, 0.032);
        tone(900 * variation, t, 0.035, 0.08 * strength, 'triangle', false, 670);
        break;
      case 'hold':
        noiseHit(t, 0.06, 1200 * variation, 0.028);
        tone(620 * variation, t, 0.075, 0.07, 'sine');
        tone(930 * variation, t + 0.025, 0.12, 0.05, 'sine');
        break;
      case 'release':
        noiseHit(t, 0.045, 900 * variation, 0.023);
        tone(710 * variation, t, 0.1, 0.055, 'sine', false, 480);
        break;
      case 'select':
      ...
      case 'turn':
        ...
        break;
      case 'yacht':
        [72, 76, 79, 84, 88].forEach((note, i) => tone(pitch(note), t + i * 0.11, 0.55, 0.07));
        break;
      case 'finish':
