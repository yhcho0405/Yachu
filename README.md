...
가입 없이 닉네임으로 시작하는 온라인 보드게임. 같은 사이트에서 야추·티카투카·아발론을 선택하고 초대 링크로 친구와 플레이합니다. Three.js 보드, 서버가 확정하는 규칙, 저장되는 경기와 재접속을 제공합니다.

| 게임     | 혼자 시작          | 온라인 방        |
| -------- | ------------------ | ---------------- |
| 야추     | 기존 1인 플레이    | 2–4인 대전       |
| 티카투카 | 서버 컴퓨터와 1대1 | 사람끼리 1대1    |
| 아발론   | 지원하지 않음      | 5–10인 정체 추리 |
...
npm run test:persistence  # 세 게임의 실제 프로세스 중지/재시작
```
...
아발론은 인원별 규칙, 선택 역할의 정보 차이, 동시 제출, 익명 원정 집계, 암살과 호수의 여인을 검사합니다. 5·10인 정상 브라우저 흐름과 실제 workerd 재시작, HTTP·WebSocket·중복 응답의 수신자별 정보 경계를 별도로 확인합니다. 원작과 웹 정책은 [아발론 규칙](docs/avalon-rules.md), 그래픽 측정과 한계는 [그래픽 검증](docs/avalon-graphics-verification.md)을 참고하세요.
...
아발론은 기본 멀린·암살자와 선택 퍼시벌·모르가나·모드레드·오베론, 기본 꺼짐인 호수의 여인을 제공합니다. 공개 채팅·의견 표시·투표 비교표와 직접 여는 개인 정보 패널을 사용합니다. 개인 메모는 해당 브라우저에만 저장됩니다. 선의 원정 3회 성공 뒤에도 암살 판정까지 최종 승리를 확정하지 않습니다.

진행 중 연결 끊김은 한 경기 전체에서 누적120초까지 허용하고 재접속으로 초기화하지 않습니다. 야추·티카투카의 명시적 퇴장은 즉시 기권입니다. 아발론은 역할·좌석을 고정하며 명시적 퇴장이나 유예 소진 시 승리 진영과 전체 역할 공개 없이 경기를 무효 종료합니다. 로비 방장은 남은 사람에게 위임하고 모든 사람이 나간 방은 짧은 유예 뒤 정리합니다. 자세한 정책은 [서버 문서](docs/server.md)에 있습니다.

## 배경 음악

로비·야추·티카투카마다 사용자가 제공한 MP3 두 곡을 사용합니다. 각 상황의 첫 곡은 무작위이며 이후 1·2를 번갈아 재생합니다. 곡의 끝과 화면 전환은 교차 페이드하고, 공통 음소거·BGM 볼륨을 적용합니다. 카드 선택·도움말·설정만 열면 음악을 다시 시작하지 않습니다. 브라우저의 첫 클릭·터치·키보드 동작 후 재생되며, 숨김과 음소거는 같은 위치를 보존합니다. 파일과 재생 수명은 [BGM 문서](docs/music.md)에 있습니다.

아발론은 제공된 `aval1.mp3` 한 곡을 반복합니다. 로비와 오갈 때와 곡이 끝날 때 같은 1.5초 교차 페이드를 적용합니다.
